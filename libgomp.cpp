...
// It spawns a pool of OMP omp_thread threads.

void libgomp_init()
    {
    for(auto task: tasks)                           // put all tasks into the idle task pool
        {
        task_pool.add(&task);
        }
...
void GOMP_task (    void (*fn) (void *),
                    ...
                    void *detach)
    {
    if(!if_clause)                              // if if_clause if false we have to run the task right now
        {
