...
#define FLT(x) (int)(x), (unsigned)((x)*1000 - (int)(x)*1000) 
#define FLTF "%3d.%03u" 
...
int main(int argc, const char **argv)
    ...
    if(argc == 3 && argv[1][0] == 's')
        {
        startSize = maxArsize = atoi(argv[2]);
        minTime = 10;
        }
    else
        {
    ...
    printf("                            Reps   Time   DGEFA     DGESL   OVERHEAD   MFLOPS\n");
    printf("                            -------------------------------------------------\n");
    for(arsize=startSize; arsize<=maxArsize; arsize += step)
        {
        arsize/=2;
        arsize*=2;
        if (arsize<10)
        ...
        // printf("mempool = %p\n", mempool);
        nreps=1;
        while (linpack(nreps,arsize)<minTime)
            {
            nreps*=2;
            }
        free(mempool);
    ...
    printf("at %d X %d, max MFLOPS =  " FLTF "\n", maxAr, maxAr, FLT(max/1000.0));
    return 0;
    }
...
static REAL linpack(long nreps,int arsize)
    ...
    if (toverhead<0.)
        toverhead=0.;
    printf("Array size %4d X %4d  ",arsize,arsize);
    printf("%8ld " FLTF " " FLTF "%% " FLTF "%% " FLTF "%%  " FLTF "\n",
            nreps,
            FLT(totalt),
            FLT(100.*tdgefa/totalt),
            FLT(100.*tdgesl/totalt),
            FLT(100.*toverhead/totalt),
            FLT(kflops/1000.0));
    if(kflops > max)
        {
        max = kflops;
